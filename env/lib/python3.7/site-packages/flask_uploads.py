...
an `` object and upload your files to it.
...
def lowercase_ext(filename):
    """
    This is a helper used by .save to provide lowercase extensions for
...
def configure_uploads(app, upload_sets):
    ...
    :param upload_sets: The `` instances to configure.
    """
...
class AllExcept(object):
    ...
    to the `` constructor as `extensions`. You can use any container,
    for example::
...
class UploadConfiguration(object):
    """
    This holds the configuration for a single ``. The constructor's
